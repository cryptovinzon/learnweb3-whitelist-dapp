...
export default function Home() {
  // walletConnected tracks if the user's wallet is connected or not
  const [walletConnected, setWalletConnected] = useState(false);
  // joinedWhitelist tracks if the current metamask address has joined the Whitelist or not
  ...
  const addAddressToWhitelist = async () => {
    try {
      ...
      // call the addAddressToWhitelist function from the smart contract
  ...
  const getNumberOfWhitelisted = async () => {
    try {
      // Get the provider from web3Modal, which in our case is MetaMask. No signer needed, only reading state from blockchain
